...
nonmem_data <- read_csv("depot_1cmt_linear_ragged/Data/depot_1cmt_prop.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
sample_and_save_all_quick <- function(grainsize, solver, run_number){
  ...
  init_files <- str_c("depot_1cmt_linear_ragged/Data/Inits/inits_", 
...
sample_and_save_all <- function(threads_per_chain, grainsize, solver, 
                                run_number){

  print(str_c("threads_per_chain = ", threads_per_chain, ", ",
              "solver = ", solver, ", ", 
              "grainsize = ", grainsize, ", ",  
              "run #", run_number))
  
  init_files <- str_c("depot_1cmt_linear_ragged/Data/Inits/inits_", run_number, 
                      "_", 1:4, ".json")
  ...
  fit$save_object(str_c("depot_1cmt_linear_ragged/Stan/Fits/Grainsize/", solver_string, 
                        "_threads_", threads_per_chain,
                        "_grainsize_", grainsize, "_run_", run_number, ".rds"))
  
}

expand_grid(threads_per_chain = c(4, 24),
            solver = c(1, 2, 3),
