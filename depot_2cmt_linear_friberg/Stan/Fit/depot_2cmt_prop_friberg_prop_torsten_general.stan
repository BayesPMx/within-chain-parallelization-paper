...
// Torsten's General ODE solver that uses MPI for within-chain parallelization
...
transformed data{ 
  ...
  array[n_obs] int cmt_obs = cmt[i_obs];
  ...
  array[n_subjects, n_cmt + n_cmt_pd] real bioav = 
                        rep_array(1.0, n_subjects, n_cmt + n_cmt_pd); // Hardcoding, but could be data or a parameter in another situation
  array[n_subjects, n_cmt + n_cmt_pd] real tlag = 
                        rep_array(0.0, n_subjects, n_cmt + n_cmt_pd);
  
  array[n_subjects] int n_each_subject;

  for(i in 1:n_subjects){
    n_each_subject[i] = subj_end[i] - subj_start[i] + 1;
  }
...
transformed parameters{
...

  {
    ...
    x_ipred = pmx_solve_group_rk45(depot_2cmt_friberg_ode,
                                   n_cmt + n_cmt_pd,
                                   n_each_subject,
                                   time,
                                   amt,
                                   ...
                                   addl,
                                   ss,
                                   to_array_2d(append_col(theta, theta_pd)), 
                                   bioav, tlag)';
  ...
  if(prior_only == 0){
    for(i in 1:n_obs){
      ...
      if(cmt_obs[i] == 2 || cmt_obs[i] == 4){
        ...
        real sigma_tmp = cmt_obs[i] == 2 ? ipred_tmp*sigma_p : ipred_tmp*sigma_p_pd;
